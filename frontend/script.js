...
function setupMobileShowcaseScroll() {
  if (window.innerWidth > 1024) return;
  const showcase = document.querySelector('.showcase-section');
  const mockup = document.querySelector('.device-mockup');
  const frames = Array.from(document.querySelectorAll('.frame'));
  const dots = Array.from(document.querySelectorAll('#frameDots span'));
  const screen = document.querySelector('.device-screen');
  if (!showcase || !mockup || frames.length === 0 || !screen) return;

  let manualIndex = 0;
  const activate = (index) => {
    frames.forEach((frame, idx) => frame.classList.toggle('active', idx === index));
  ...
  const onScroll = () => {
    ...
    const progress = Math.min(1, Math.max(0, (viewport * 0.75 - rect.top) / (viewport * 1.4)));
    const autoIndex = Math.min(frames.length - 1, Math.floor(progress * frames.length));
    const currentIndex = Math.max(autoIndex, manualIndex);
  ...
  screen.addEventListener(
    ...
    (event) => {
      ...
      if (delta < 0) {
        manualIndex = Math.min(frames.length - 1, manualIndex + 1);
      } else {
        manualIndex = Math.max(0, manualIndex - 1);
      }
      activate(manualIndex);
    },
    { passive: true }
  );
...
function setupRevealAnimations() {
  const revealItems = document.querySelectorAll(
    '.stat-card, .service-card, .module-card, .plan-card, .review-card, .google-card, .timeline-card, .policy-card'
  );
  revealItems.forEach((el, index) => {
    el.classList.add('reveal');
    el.style.setProperty('--reveal-delay', `${index % 8 * 50}ms`);
  });
